...
def load_rules(rule_sets: List[str], apply_generic: bool, extra_rule_dirs: List[Path] = []) -> List[IDSValidationRule]:
  """"""
  rule_set_dirs = discover_rulesets(extra_rule_dirs=extra_rule_dirs)
  filtered_dirs = filter_rule_sets(rule_set_dirs, rule_sets=rule_sets, apply_generic=apply_generic)
...
def filter_rule_sets(rule_set_dirs: List[Path], rule_sets: List[str], apply_generic: bool) -> List[Path]:
  pass
